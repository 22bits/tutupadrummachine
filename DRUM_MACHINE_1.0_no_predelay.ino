...
const int  MIN_STEP_TIME          = 120;
const int  MAX_STEP_TIME          = 550;
const byte BRIGHTNESS             = 4; // From 0 to 15
const byte HIT_SERVO_ANGLE        = 70;
const byte MIN_SERVO_START_ANGLE  = 5; // Never set to 0. May be outside of the range of some servos!
const byte MAX_SERVO_START_ANGLE  = 30;
const int  RESET_PRESS_TIME       = 1000;
const byte RND_SEQ_THRSHLD        = 75;
const byte CLK_SIGNAL_TIME        = 15;


const byte SERVO_UPDATE_INT       = 10;
const byte SERVO_INCREMENT        = 7;
...
#define servo_angle_in        A1

#define CLOCK_IN_THRESHOLD    750  // A value from 0 to 1023, which represents the range from 0 to VCC

// Servo pins -----------------------
const int servoPin[TOTAL_TRACKS]        = { 11, 10, 3, 2};

// Button mapping (4021) ------------
//const byte step_buttons[TOTAL_STEPS]    = { 7, 6, 5, 4, 3, 2, 1, 0};
//const byte track_buttons[TOTAL_TRACKS]  = { 8, 9, 10, 11, 12, 13};
...
volatile int            initialServoPos = MIN_SERVO_START_ANGLE;
  ...
  void update(){
    ...
    if (moving){
      if((curTime - lastUpdate) >= updateInterval){
        lastUpdate = curTime;
        targetPos = constrain(pos + incstep, initialServoPos, HIT_SERVO_ANGLE+1);
      ...
      setTargetPos max(targetPos-2, initialServoPos);
    }
  }
};
...
void loop() {
  ...
  initialServoPos = MIN_SERVO_START_ANGLE;
#else
  // Read the speed selection pot
  stepInterval = map(analogRead(speed_analog_in), 0, 1023, MIN_STEP_TIME, MAX_STEP_TIME);
  initialServoPos = map (analogRead(servo_angle_in), 0, 1023, MIN_SERVO_START_ANGLE, MAX_SERVO_START_ANGLE);
#endif
